        ...
        try:
            ...
            # Subscription preapproval notification
                ...
                if subscription_status == "authorized" and external_ref:
                    ...
                    plan = None  # You can extract plan from subscription_data if needed
                    # For now, we'll just mark the user as active with a generic plan
                    subscribers = load_subscribers()
                    if chat_id not in subscribers:
                        subscribers[chat_id] = {}
                    subscribers[chat_id]["subscription_id"] = subscription_id
                    subscribers[chat_id]["status"] = "active"
                    subscribers[chat_id]["active"] = True
                    subscribers[chat_id]["plan"] = "premium"  # or map to actual plan
                    save_subscribers(subscribers)
                    logger.info(f"✅ Subscription {subscription_id} activated for user {chat_id}")
                    send_telegram(int(chat_id), "✨ Your Premium subscription is now ACTIVE! Thank you for subscribing.")
            return "OK", 200
        except Exception as e:
